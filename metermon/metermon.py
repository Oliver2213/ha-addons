...
MQTT_BROKER_HOST  = os.getenv('MQTT_BROKER_HOST',"homeassistant.local")
MQTT_BROKER_PORT  = int(os.getenv('MQTT_BROKER_PORT',1883))
MQTT_CLIENT_ID    = os.getenv('MQTT_CLIENT_ID',"metermon")
MQTT_USERNAME     = os.getenv('MQTT_USERNAME',"addons")
MQTT_PASSWORD     = os.getenv('MQTT_PASSWORD',"")
MQTT_TOPIC_PREFIX = os.getenv('MQTT_TOPIC_PREFIX',"metermon")
RTL_TCP_SERVER    = os.getenv('RTL_TCP_SERVER',"homeassistant.local:1234")
...
METERMON_SEND_BY_ID = os.getenv('METERMON_SEND_BY_ID', "True")
...
while True:
    ...
    # set Protocol
    try:
        msg['Protocol'] = data['Message']['EndpointType']
    except KeyError:
        # rtlamr doesn't send the protocol key anymore it seems.
        # So I've inferred based on the meter I see and the two protocols it uses which one is in use.
        if data['Message'].get('Preamble'):
          msg['Protocol'] = 'IDM'
        else:
          msg['Protocol'] = 'SCM+'
    ...
    if msg['Consumption'] > 0:        
        ...
        if METERMON_SEND_BY_ID.lower() == "true":
            ...
            print("Sending to mqtt topic {}".format(MQTT_TOPIC_PREFIX+"/"+msg['ID']))
